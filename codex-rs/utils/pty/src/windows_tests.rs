...
async fn assert_terminate_kills_descendant(
    ...
    // Immediate descendants must join the job before either backend starts the root.
    let code = format!(
...
async fn terminate_kills_immediate_descendants_for_pipe_and_conpty() -> anyhow::Result<()> {

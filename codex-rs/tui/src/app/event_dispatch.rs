    ...
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::ApplyBacktrackRestore {
                ...
                Box::pin(self.handle_apply_backtrack_restore(
                    tui,
                    app_server,
                    super::backtrack_events::BacktrackRestoreRequest {
                        thread_id,
                        nth_user_message,
                        target,
                        prompt,
            ...
            AppEvent::UpdateApprovalsReviewer(policy) => {
                ...
                // Permission mode changes are session-scoped. Writing config.toml here would
                // reload every thread's user config and rebuild MCP for a local mode switch.
                self.runtime_permission_profile_override =
                    Some(RuntimePermissionProfileOverride::from_config(&self.config));
                self.sync_active_thread_permission_settings_to_cached_session()
                    .await;
